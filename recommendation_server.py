...
2. **The recent-id cache is bounded.** `get_recommendations` records requested ids
   into a module-level `deque(maxlen=_SEEN_IDS_MAXLEN)`, so the tracking structure
   cannot grow without limit under sustained load. `test_memory_is_bounded` pins
   this down.
...
# stays flat under sustained load. Must never be replaced by an unbounded
# container (that was the f18f452 regression -> monotonic RSS growth -> OOM).
...
def get_recommendations(input_product_ids: list[str], max_results: int = 5) -> list[str]:
    ...
    _request_count += 1
    # record recently seen ids; bounded by the deque's maxlen
...
# This keeps the memory behaviour observable at runtime rather than only in
# tests. Imported as a module (by pytest) none of this runs.
# --------------------------------------------------------------------------
...
def _background_load(rps: float) -> None:
    ...
    Each tick pushes ids through the bounded _seen_product_ids cache, so RSS in
    `docker stats` should stay flat instead of climbing."""
    i = 0
    interval = 1.0 / rps if rps > 0 else 0.01
    pad = "x" * 256  # non-trivial payload per entry, as in real traffic
    while True:
