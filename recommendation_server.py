...
2. `get_recommendations` records every requested id into a bounded
   `collections.deque(maxlen=128)` so the working set stays capped under
   sustained load. `test_memory_is_bounded` guards this invariant.

Kept dependency-light (stdlib only, no gRPC/framework) so it can be built+tested
...
class _Handler(BaseHTTPRequestHandler):
    def do_GET(self):  # noqa: N802 (stdlib API)
        if self.path.startswith("/recommend"):
            ...
            body = (
                "# HELP recommendation_seen_ids_total tracked product ids (bounded cache size)\n"
...
def _background_load(rps: float) -> None:
    """Continuously exercise the service so it behaves like real traffic (no
